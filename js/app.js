document.addEventListener("DOMContentLoaded", () => {
  ...
  fetch("data/products.json")
    ...
    .then(products => {
      ...
      // ── Hero spotlight: produk diskon terbesar (data-driven, bukan hardcode) ──
      const fmtBRL0 = v => "R$ " + v.toLocaleString("pt-BR", { minimumFractionDigits: 2, maximumFractionDigits: 2 });
      try {
        const spot = [...products].sort((a, b) => (parseInt(b.discount || 0, 10) - parseInt(a.discount || 0, 10)))[0];
        if (spot) {
          const set = (id, v) => { const el = document.getElementById(id); if (el) el.textContent = v; };
          const img = document.getElementById("hero-spot-img");
          if (img) { img.src = imgVariant(spot.image, 960); img.alt = spot.name; }
          set("hero-spot-name", spot.name);
          set("hero-spot-price", spot.price);
          set("hero-spot-orig", spot.originalPrice || "");
          set("hero-spot-off", spot.discount ? `-${spot.discount}%` : "");
          try {
            const toN = s => parseFloat(String(s||"").replace("R$","").trim().replace(/\./g,"").replace(",",".")) || 0;
            const sv = Math.max(0, toN(spot.originalPrice) - toN(spot.price));
            const hs = document.getElementById("hero-spot-save");
            const ht = document.getElementById("hero-spot-save-text");
            if (hs && ht) { if (sv >= 5) { ht.textContent = "Economize " + fmtBRL0(sv); hs.style.display = ""; } else hs.style.display = "none"; }
          } catch (err) {}
          const card = document.getElementById("hero-spot");
          if (card) {
            const go = () => openSheet(spot.id);
